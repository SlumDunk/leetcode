...
public class Leetcode833 {
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        ...
        for (int i = 0; i < indexes.length; i++) {
            ...
            //置位，表示已经处理过
            indexes[minIndex] = Integer.MAX_VALUE;
            res.append(S.substring(last, min));
            last = min;
            if (S.substring(min, min + sources[minIndex].length()).equals(sources[minIndex])) {
                res.append(targets[minIndex]);
                last = min + sources[minIndex].length();
            }
        }
